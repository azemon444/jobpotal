                ...
                @if(Auth::check() && Auth::user()->hasRole('admin'))
                  <div class="d-flex flex-column flex-md-row gap-2 mt-3 mt-md-0">
                    <form action="{{ route('admin.approvePost', $post->id) }}" method="POST" style="display:inline-block;">
                      @csrf
                      <button class="btn btn-success px-4" style="border-radius:2em; font-weight:500;" type="submit">Approve</button>
                    </form>
                    <form action="{{ route('admin.rejectPost', $post->id) }}" method="POST" style="display:inline-block;">
                      @csrf
                      <button class="btn btn-danger px-4" style="border-radius:2em; font-weight:500;" type="submit">Reject</button>
                    </form>
                  </div>
                  <div class="card mt-4" style="max-width:500px;">
                    <div class="card-header bg-primary text-white">Admin Comment</div>
                    <div class="card-body">
                      <form action="{{ route('admin.sendFeedback', $post->id) }}" method="POST">
                        @csrf
                        <textarea name="admin_feedback" class="form-control mb-2" rows="2" placeholder="Leave a comment for the author...">{{ $post->admin_feedback }}</textarea>
                        <button class="btn btn-warning w-100" type="submit">Send Comment</button>
                      </form>
                      @if($post->admin_feedback)
                        <div class="mt-3"><strong>Admin:</strong> {{ $post->admin_feedback }}</div>
                      @endif
                      @if($post->author_reply)
                        <div class="mt-2"><strong>Author:</strong> {{ $post->author_reply }}</div>
                      @endif
                    </div>
                  </div>
                @endif
