                ...
                @if(Auth::check() && Auth::user()->hasRole('admin'))
                  ...
                  <div class="admin-action-modern card shadow-sm border-0 p-4 mb-4" style="max-width:540px; background: #f8fafc;">
                    <div class="d-flex flex-column flex-md-row align-items-center justify-content-between gap-3 mb-3">
                      <form action="{{ route('admin.approvePost', $post->id) }}" method="POST" class="w-100 me-md-2">
                        @csrf
                        <button class="btn btn-success w-100 py-2 fw-semibold rounded-pill" type="submit"><i class="fas fa-check-circle me-2"></i>Approve</button>
                      </form>
                      <form action="{{ route('admin.rejectPost', $post->id) }}" method="POST" class="w-100 ms-md-2">
                        @csrf
                        <button class="btn btn-danger w-100 py-2 fw-semibold rounded-pill" type="submit"><i class="fas fa-times-circle me-2"></i>Reject</button>
                      </form>
                    </div>
                    <form action="{{ route('admin.sendFeedback', $post->id) }}" method="POST" class="mt-2">
                      @csrf
                      <label for="admin_feedback" class="form-label fw-bold text-primary">Admin Comment</label>
                      <textarea name="admin_feedback" id="admin_feedback" class="form-control mb-2 rounded-3" rows="3" placeholder="Leave a comment for the author..." style="resize:vertical;"></textarea>
                      <button class="btn btn-warning w-100 py-2 fw-semibold rounded-pill" type="submit"><i class="fas fa-paper-plane me-2"></i>Send Comment</button>
                    </form>
                  </div>
                  @endif
