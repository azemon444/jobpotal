  ...
  <div class="account-layout  border">
    <div class="account-hdr text-white border-0 text-center py-4 position-relative" style="background: linear-gradient(135deg, #0dcaf0 60%, #0a58ca 100%); border-radius: 1.5rem 1.5rem 0 0; box-shadow: 0 4px 24px rgba(13,202,240,0.08);">
            ...
            <div class="d-flex justify-content-center align-items-center gap-2 mt-2">
                <a href="{{ route('account.index') }}" class="btn btn-outline-light btn-sm" style="font-weight:500; border: 1.5px solid #fff; background: transparent;"><i class="fas fa-user"></i> View Profile</a>
                @if(Auth::user() && (Auth::user()->hasRole('employer') || Auth::user()->hasRole('author') || Auth::user()->hasRole('admin')))
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @if($company)
                            ...
                            <tr>
                                <td>{{$index+1}}</td>
                                <td> <a href="{{route('post.show',['job'=>$post])}}" target="_blank" title="Go to this post">{{$post->job_title}}</a>
                                    ...
                                    @if($post->author_reply)
                                        <div class="alert alert-info p-2 mb-1"><strong>Your Reply:</strong> {{ $post->author_reply }}</div>
                                    @endif
                                </td>
