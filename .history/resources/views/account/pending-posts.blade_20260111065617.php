                    ...
                    <tbody style="text-align:center; vertical-align:middle;">
                        ...
                        <tr>
                            ...
                            <td style="text-align:left;">
                                <div style="display: flex; flex-direction: column; align-items: flex-start; gap: 6px; justify-content: flex-start;">
                                    <a href="{{ route('post.show', $post->id) }}" class="btn btn-info btn-sm px-3 mb-1" style="border-radius:2em; font-weight:500; width:100%;">View</a>
                                    <form action="{{ route('admin.approvePost', $post->id) }}" method="POST" style="display:inline-block; width:100%;">
                                        @csrf
                                        <button class="btn btn-success btn-sm px-3 mb-1" style="border-radius:2em; font-weight:500; width:100%;">Approve</button>
                                    </form>
                                    <form action="{{ route('admin.rejectPost', $post->id) }}" method="POST" style="display:inline-block; width:100%;">
                                        @csrf
                                        <button class="btn btn-danger btn-sm px-3 mb-1" style="border-radius:2em; font-weight:500; width:100%;">Reject</button>
                                    </form>
                                    <form action="{{ route('admin.sendFeedback', $post->id) }}" method="POST" style="min-width:120px; max-width:160px; margin-top:0; display:inline-block; width:100%;">
                                        @csrf
                                    ...
                                    <div style="width:100%;">
                                        @if($post->admin_feedback)
                                            <span class="badge bg-warning text-dark" style="font-size:0.97em; white-space:normal;"><strong>Admin:</strong> {{ $post->admin_feedback }}</span>
                                        @endif
                                        @if($post->author_reply)
                                            <span class="badge bg-info text-dark" style="font-size:0.97em; white-space:normal;"><strong>Author:</strong> {{ $post->author_reply }}</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
