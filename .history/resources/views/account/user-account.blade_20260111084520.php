		...
		<div class="col-md-4 col-12 mb-4 mb-md-0">
			<div class="profile-sidebar p-4 border shadow-sm h-100" style="background:#0dcaf0; color:#fff;">
				<div class="text-center mb-3">
				   ...
				   <h4 class="fw-bold text-center mb-1" style="color:#fff;">{{ isset($user) ? ($user->first_name ?? ($user->getFirstName() ?? $profile->full_name ?? '')) : (auth()->user()->first_name ?? (method_exists(auth()->user(), 'getFirstName') ? auth()->user()->getFirstName() : $profile->full_name ?? '')) }}</h4>
				   <div class="text-center mb-3">
					   <span class="badge bg-white text-primary px-3 py-2 mb-2" style="font-size:1em; font-weight:600;">{{ isset($user) ? $user->email : auth()->user()->email }}</span>
				   </div>
