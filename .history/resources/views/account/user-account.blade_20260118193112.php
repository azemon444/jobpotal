	...
	<div class="row justify-content-end mb-3">
		<div class="col-auto">
			<div class="dropdown">
				...
				<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="notificationsDropdown" style="min-width: 320px; max-height: 350px; overflow-y: auto;">
					@forelse(auth()->user()->unreadNotifications as $notification)
						<li class="px-3 py-2 border-bottom small">
							<div>{{ $notification->data['message'] ?? '' }}</div>
							<div class="text-muted" style="font-size: 0.85em;">{{ $notification->created_at->diffForHumans() }}</div>
							<form method="POST" action="{{ route('notifications.markAsRead', $notification->id) }}" class="d-inline">
								@csrf
								<button type="submit" class="btn btn-link btn-sm p-0 text-success">Mark as read</button>
							</form>
						</li>
					@empty
					...
					<div class="col-12">
						...
						<div class="card shadow-sm rounded-4 mb-4">
							<div class="card-body">
								...
								<div class="ps-1">
									<span class="badge bg-warning bg-opacity-25 text-warning fw-semibold" style="font-size:1em; border-radius:1em;">{{ $profile->email }}</span>
								</div>
							</div>
						</div>
						@endif
