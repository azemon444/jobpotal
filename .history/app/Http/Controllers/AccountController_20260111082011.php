...
{
    // Show jobseeker profile for admin view
    public function jobseekerProfile($user)
    {
        $profile = \App\Models\UserProfile::firstOrNew(['user_id' => $user]);
        $userObj = \App\Models\User::find($user);
        return view('account.user-account', compact('profile', 'userObj'));
    }
