...
class AccountController extends Controller
{
    ...
    $user = User::find(auth()->user()->id);
        ...
        // Notify employer with direct application link
        $post = Post::find($request->post_id);
        if ($post && $post->company && $post->company->user) {
            $employer = $post->company->user;
            $applicationUrl = url('/account/job-application/' . $application->id);
            $employer->notify(new \App\Notifications\NewJobApplicationNotification($user->name, $post->job_title, $applicationUrl));
        }
